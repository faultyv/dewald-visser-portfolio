        ...
        <StaggerGroup className="method-runway relative grid gap-3 rounded-[28px] border border-outline-variant bg-surface-container-low p-3 md:grid-cols-2 md:gap-3.5 md:p-4">
          {steps.map((step, i) => (
            <StaggerItem key={step.title}>
              <article className="method-step-card hig-card min-h-[178px] rounded-[22px] p-4.5 md:min-h-[210px] md:p-5">
                ...
                <div className="relative z-10 mb-5 flex items-start justify-between gap-3 md:mb-7">
                  ...
                  <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${SEED_BG[step.seed]} ${SEED_ON[step.seed]} elevation-1`}>
                    <IconSymbol name={STEP_ICONS[i] ?? "route"} size={23} filled />
                  </div>
                </div>
